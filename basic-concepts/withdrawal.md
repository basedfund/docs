...
In oder to avoid a bank run during an adverse event we have implemented a withdraw delay. The protocol includes a fee free period (labeled as _**Pf**_) and a withdraw delay that helps our contracts understand when to avoid charging a fee.
...
* _**A**_ is the amount of the request.
...
1. A user sends a withdraw request to safe123with \[amount A , delay D]
2. safe123 checks the users balance and reduces it by A if the request is valid
